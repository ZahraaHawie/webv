...
export default function LeadershipPageClientFallback({ articles, serviceSlug }: Props) {
  ...
  const filteredArticles = selectedFilter === 'All'
    ...
    : articles.filter(article => article.tag === selectedFilter); // Show all matching articles for specific filters
          ...
          <div className="w-1/2 h-full relative overflow-hidden">
            <div className="absolute inset-0 z-0">
              <video ref={videoRef} autoPlay loop muted playsInline className="w-full h-full object-cover">
                <source src="/media/leadership.mp4" type="video/mp4" />
              </video>
              <div className="absolute inset-0 bg-black/30" />
            </div>
          </div>
          <div className="w-1/2 h-full flex flex-col justify-center p-16">
            <div className="text-gray-700 italic mb-4" style={{ fontFamily: 'Ghost', fontSize: 20, fontWeight: 400, fontStyle: 'italic' }}>Leadership Insights</div>
            ...
            <p className="text-gray-700 max-w-lg" style={{ fontFamily: 'Lausanne', fontSize: 'clamp(16px, 1.2vw, 20px)', lineHeight: 1.6, fontWeight: 400 }}>
              Discover leadership insights, strategies, and stories that shape the future of organizations and drive meaningful change in today&apos;s dynamic business landscape.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16">
          <div className="mb-16">
            ...
            <div className="flex flex-wrap justify-between items-center mb-8">
              <div className="flex flex-wrap gap-4">
                {['All', 'Global News', 'Market Reports', 'Strategic Insights', 'Case Studies'].map((filter) => (
                  <button
                    key={filter}
              ...
              <div className="relative filter-dropdown-container">
                ...
                {isFilterDropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                    <div className="py-2">
                      {['Global News', 'Strategic Insights', 'Market Reports', 'Case Studies'].map((category) => (
                        <button
                          key={category}
                          onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {filteredArticles.map((article, index) => (
              <Link key={index} href={`/services/${serviceSlug}/Leadership/${article.slug}`}>
                <motion.div
